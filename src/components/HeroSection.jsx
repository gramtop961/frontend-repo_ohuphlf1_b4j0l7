...
export default function HeroSection({ lang = 'en', onGetQuote }) {
  ...
  return (
    <section className="relative min-h-[88vh] w-full overflow-hidden bg-[#0D0D10] text-[#E8E8EC]">
      {/* Spline cover background */}
      <div className="absolute inset-0">
        <Spline scene="https://prod.spline.design/cEecEwR6Ehj4iT8T/scene.splinecode" style={{ width: '100%', height: '100%' }} />
      </div>

      {/* Overlays: gradients & textures (do not block Spline interactions) */}
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        <div className="absolute inset-0 opacity-[0.10] mix-blend-screen" style={{
          backgroundImage:
        ...
        <div className="absolute -top-24 -right-24 h-96 w-96 rounded-full opacity-30 blur-3xl" style={{
          background: 'linear-gradient(135deg, #FF7A00, #FFD23A)'
        }} />
        <div className="absolute -bottom-24 -left-24 h-96 w-96 rounded-full opacity-25 blur-3xl" style={{
      ...
      <div className="relative z-10">
        <div className="mx-auto flex max-w-7xl flex-col gap-8 px-6 pt-36 pb-16 md:px-10">
          ...
          <div className="relative">
            ...
            >
              <span className="block text-[#E8E8EC] drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)]">{t.h1Top}</span>
              <span className="block bg-clip-text text-transparent drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)]" style={{ backgroundImage: 'linear-gradient(135deg, #FF7A00, #FFD23A)' }}>{t.h1Bottom}</span>
            </motion.h1>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="max-w-2xl text-lg text-[#E8E8EC]/85">
            {t.sub}
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="flex flex-wrap items-center gap-4">
            ...
            <a href={whatsappLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#E8E8EC]/20 bg-[#17181C]/80 px-5 py-3 text-[#E8E8EC] backdrop-blur-sm transition hover:bg-[#1d1f24]">
              <MessageCircle className="h-4 w-4 text-[#70E000]" />
              {t.ctaChat}
            </a>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="mt-4 text-sm text-[#E8E8EC]/70 drop-shadow-[0_2px_6px_rgba(0,0,0,0.6)]">
            {t.trust}
          </motion.div>

          <div className="mt-8 text-xs tracking-widest text-[#E8E8EC]/50">{t.brand}</div>
        </div>
      </div>
    </section>
  );
